...
function createUploader() {
  ...
  const uploadProgressCb = ({ total, loaded, chunks }) => {
    const percentage = Math.round(((loaded * 100) / total) / chunks);
...
function* handleUploadFileRequest(action) {
  ...
  try {
    ...
    for (var i = 0; i < chunks.length; i++) {
      let res = yield call(uploadFile, {...action.payload, file: chunks[i], index: i+1, offset: i*chunks[i].size, total: chunks.length, uuid}, uploadProgressCb);
    }
